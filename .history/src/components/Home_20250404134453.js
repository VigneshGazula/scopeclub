...
function Home() {
  ...
  useEffect(() => {
    ...
    const splineFallback = document.querySelector('.spline-fallback');
    
    if (splineViewer && splineFallback) {
      // Listen for global Spline script load error
      const handleGlobalScriptError = () => {
        console.log('Spline script failed to load, showing fallback content');
        if (splineViewer) {
          splineViewer.classList.add('error');
        }
        if (splineFallback) {
      ...
      document.addEventListener('spline-load-error', handleGlobalScriptError);
      
      // Check if Spline URL is accessible
      const checkSplineUrl = async () => {
        try {
          const splineUrl = splineViewer.getAttribute('url');
          const response = await fetch(splineUrl, { method: 'HEAD' });
          return response.ok;
        } catch (error) {
          console.error('Error checking Spline URL:', error);
          return false;
        }
      };
      ...
      const handleSplineError = () => {
        console.log('Spline loading failed, showing fallback content');
        splineViewer.classList.add('error');
        splineFallback.style.display = 'flex';
      };
      ...
      const handleSplineLoad = () => {
        ...
        splineViewer.classList.remove('error');
        splineFallback.style.display = 'none';
      };
      
      // Check URL availability before loading
      checkSplineUrl().then(isAvailable => {
        if (!isAvailable) {
          handleSplineError();
        } else {
          // Add event listeners
          ...
          // Set a timeout to handle case where Spline viewer doesn't fire events
          const timeout = setTimeout(() => {
            if (!splineViewer.loaded && !splineViewer.error) {
              handleSplineError();
            }
      ...
      return () => {
        ...
        document.removeEventListener('spline-load-error', handleGlobalScriptError);
      };
    }
  }, []);
      ...
      <div className="inside-scope">
        ...
        <div className="is-top-layer">
          ...
          <script type="module" src="https://unpkg.com/@splinetool/viewer@1.9.68/build/spline-viewer.js"></script>
            <spline-viewer 
              url="https://prod.spline.design/gfHtOWImHKL9KP6O/scene.splinecode"
              loading-anim="true"
              loading-progress="true"
              class="spline-viewer"
              events-target="global"
              onError={(e) => console.error("Spline loading error:", e)}
            ></spline-viewer>
            <div className="spline-fallback" style={{display: 'none'}}>
              <img src={`${process.env.PUBLIC_URL}/images/scope-logo.png`} alt="Scope Logo" />
            </div>
          </div>
        </div>
      </div>
